...
def build_players(batting_rows, pitching_rows,
    ...
    for norm in all_norms:
        ...
        if not_eligible:
            # Player exists but hasn't met PA/IP threshold yet
            adj_value = espn_value   # pure ESPN until eligible
            kev_score = round(espn_value * 0.5, 4) if espn_value > 0 else 0
        else:
            adj_value = adjusted_espn_value(espn_value, rs)
            kev_score = round(0.8 * adj_value + 0.2 * rs, 4)
...
def main():
    ...
    github_put_file(DAILY_SNAPSHOT_FILE,
                    ...
                    daily_sha if saved_on != today_str else None,
