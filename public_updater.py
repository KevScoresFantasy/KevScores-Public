...
MIN_IP = 10.0  # minimum innings pitched to get a rank score (pitchers)
...
def build_players(batting_rows, pitching_rows,
    ...
    for norm in all_norms:
        ...
        adj_value = adjusted_espn_value(espn_value, rs if not not_eligible else 0)
        if not_eligible:
            if espn_rank < 900:
                # Has ESPN rank — use ESPN value directly, scaled down slightly
                kev_score = round(adj_value, 4)
            else:
                # Unknown player, no ESPN rank, no stats yet
                kev_score = 0
        else:
            # Fully eligible — adj_value IS the final weighted score
            kev_score = round(adj_value, 4)

        if kev_score < 0.1 and espn_rank >= 900 and not_eligible:
            continue   # completely unknown player with no stats yet
...
def inject(html, players, overall):
    ...
    html  = re.sub(r'const PLAYERS = \[.*?\];(?=\s*const OVERALL)',
                   lambda m: f'const PLAYERS = {json.dumps(players)};', html, flags=re.DOTALL)
    html  = re.sub(r'const OVERALL = \[.*?\];(?=\s*\n// Weighted)',
                   lambda m: f'const OVERALL = {json.dumps(overall)};', html, flags=re.DOTALL)
    return html
...
def main():
    ...
    # Always pass SHA if we have it (required to update existing file)
    ...
    if datetime.now().weekday() == 0:
        ...
        github_put_file(WEEKLY_SCORES_FILE,
                        ...
                        weekly_sha,  # None is fine here if file doesn't exist yet
